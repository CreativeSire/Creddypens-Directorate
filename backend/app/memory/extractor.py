...
class MemoryExtractor:
    ...
    def extract_from_messages(self, *, messages: list[dict[str, str]], agent_code: str | None = None) -> list[dict[str, Any]]:
        if not messages:
            return []
        extracted = self._extract_via_llm(messages=messages, agent_code=agent_code)
        if extracted:
            return extracted
        return self._extract_heuristic(messages)

    def _extract_via_llm(self, *, messages: list[dict[str, str]], agent_code: str | None) -> list[dict[str, Any]]:
        ...
        try:
            ...
            result = get_multi_llm_router().execute(
                LLMRequest(
            ...
            output.append(
                {
                    ...
                    "memory_key": "tone",
                    "memory_value": tone_match.group(1).lower(),
                    "confidence": 0.62,
                }
            )
            ...
            output.append(
                {
                    ...
                    "memory_value": format_match.group(1).lower(),
                    "confidence": 0.58,
                }
            )
        return output
